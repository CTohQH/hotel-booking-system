...
public class Booking {
    ...
    // Counter for rooms booked by each user type
    private int vipRoomsBooked;
    private int normalRoomsBooked;
    private int nonRoomsBooked; 
    ...
    public int getVipRoomsBooked() {
        return vipRoomsBooked;
    }

    public void setVipRoomsBooked(int vipRoomsBooked) {
        this.vipRoomsBooked = vipRoomsBooked;
    }

    // Getter and setter for normalRoomsBooked
    public int getNormalRoomsBooked() {
        return normalRoomsBooked;
    }

    public void setNormalRoomsBooked(int normalRoomsBooked) {
        this.normalRoomsBooked = normalRoomsBooked;
    }

    public int getNonRoomsBooked() {
        return nonRoomsBooked;
    }

    public void setNonRoomsBooked(int nonRoomsBooked) {
        this.nonRoomsBooked = nonRoomsBooked;
    }

    // Books a room for a user
    public void setBooking(User user) {
        // Validate user input
        if (user == null || user.getName() == null || user.getName().isEmpty() || user.getMemberType() == null || user.getMemberType().isEmpty()) {
        ...
        if (roomAvailable) {
            ...
            updateRoomCounter(user.getMemberType());
        } else {
    ...
    void updateRoomCounter(String memberType) {
        switch (memberType) {
            case "VIP":
                vipRoomsBooked++;
                break;
    ...
    public String getRoomType(User user) {
        ...
        if ((memberType.equals("VIP") && vipRoomsBooked >= 3) ||
            (memberType.equals("normal") && normalRoomsBooked >= 2) ||
            (memberType.equals("non-member") && nonRoomsBooked >= 1)) {
        ...
        switch (memberType) {
            case "VIP":
                ...
                if (vipRoomsBooked < 3 && room.checkRoom("VIP")) {
                    return "VIP";
            ...
            case "normal":
                ...
                if (user.getReward() && room.checkRoom("VIP")) {
                    return "VIP";
                } else if (normalRoomsBooked < 2 && room.checkRoom("Deluxe")) {
                    return "Deluxe";
                } else if (room.checkRoom("Standard")) {
                    return "Standard";
                } else {
            ...
            case "non":
                ...
                if (nonRoomsBooked < 1 && room.checkRoom("Standard")) {
                    return "Standard";
                } else {
                    throw new IllegalStateException("No rooms available for booking!");
                }
